+++
title = "वाजपेयफल-स्नान-योगः"
id = "vAjapEyaphala-snAna-yOgaH"
slug = "vajapeyaphala-snana-yogah"
source_path = "time_focus/vaara_conditioned/vAjapEyaphala-snAna-yOgaH.toml"
tags = ["RareDays", "Combinations"]
month = ["Caitra"]
category = ["Calendrical Specials"]
kaala = "sunrise"
timing_summary = "Caitra (चैत्र) māsa, at sunrise"

shlokas = '''
पुनर्वसुबुधोपेता चैत्रे मासि सिताष्टमी।
प्रातस्तु विधिवत्स्नात्वा वाजपेयफलं लभेत्॥
'''

description_en = '''
When the asterism Punarvasu occurs on a Wednesday in Chaitra (lunar) month, on a Shukla Paksha Ashtami, one obtains the great merit of performing a Vajapeya, by performing a snana in the morning.
'''

[names]
sa = ["वाजपेयफल-स्नान-योगः"]
+++
