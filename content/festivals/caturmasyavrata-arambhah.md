...
"references" = ["Smriti Kaustubham p.139-141"]
...
description_en = '''
The special Chaaturmasya vratam begins on Ekadashi, coinciding with Shayana Ekadashi. In each of the months, specific vratams are observed, with restrictions on various foods. On each of the next four shuddha dvadashis, beginning with tomorrow, a vratam starts. In `ASADha-zravaNa`, `zAkavratam` is observed; in `zravaNa--bhAdrapada`, `dadhi-vratam` is observed; in `bhAdrapada-Azvina`, `payO-vratam` is observed, and in `Azvina--kArtika`, `dvidala-vratam` is observed.
Pradakshina to Pippala; Deeparadhana in temple; Saraswati Pooja.
'''

[names]
sa = ["चातुर्मास्यव्रत-आरम्भः"]
+++
