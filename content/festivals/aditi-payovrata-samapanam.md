+++
title = "पयोव्रत-समापनम्"
id = "aditi-payOvrata-samApanam"
id_display = "aditi-payovrata-samāpanam"
slug = "aditi-payovrata-samapanam"
source_path = "general/lunar_month/tithi/12/12/aditi-payOvrata-samApanam.toml"
tags = ["VratamEnd", "SpecialPeriodEnd", "LessCommonFestivals"]
month = ["12. Phālguna"]
tithi = ["Dvādaśī"]
category = ["General Observances"]
timing_summary = "Phālguna (फाल्गुन) māsa, Śukla Pakṣa Dvādaśī"

shlokas = '''
फाल्गुनस्यामले पक्षे द्वादशाहं पयोव्रतम्।
अर्चयेदरविन्दाक्षं भक्त्या परमयाऽऽन्वितः॥
'''

description_en = '''
`payOvratam` observed by Aditi Devi ends today.
'''

[names]
sa = ["पयोव्रत-समापनम्"]
+++
