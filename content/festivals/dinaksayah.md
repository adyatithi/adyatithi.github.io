...
"references" = ["Smriti Muktaphalam Varnashrama Dharma Kanda"]
...
description_en = '''
A day within which two consecutive tithis end is called `dinakSaya`. On such days, `dAna`, `japa`, `snAna`, `hOma` yield many-fold `puNya`. Such days are also good to propitiate the `pitR`-s, and for performing `parAyaNam` of `dhruvacaritam` from `zrImadbhAgavatam` etc.
'''
